						...
						<c:forEach var="synset" items="${synsetList}" varStatus="loop">
							<tr>
								...
								<td><input type="button" class="button" value="Relations"
									onclick="window.location.href='EditRelationship?action=EditRelationship&type=<c:out value="${type}"/>&id=<c:out value="${synset[0].getOffset()}"/>'" />
								</td>
							</tr>
						</c:forEach>
						<tr>
				<td><input type="button" class="button"
					value="Insert New Synset" onclick="window.location.href='InsetNewSynset${type}?action=InsertASynset&type=<c:out value="${type}"/>&id=<c:out value="${parentId}"/>'" /></td>
					</tr>
					</c:when>
					<c:otherwise>
						<tr>
